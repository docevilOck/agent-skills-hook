...
- 若使用 `mcp__semble__`，每次调用都必须显式传 `repo`，值为当前目标仓库根目录的绝对路径或明确的远端 URL。
- 不得假设 `mcp__semble__` 会继承 cwd；也不得依赖启动时 default index。

### Workflow
- 先用 `semble search` 找到最相关代码块。
- 只有当返回的代码块上下文不足时，才打开整个文件。
- 找到一个有价值的位置后，可继续用 `semble find-related` 找相似或相关实现。
- 使用 `mcp__semble__.search` / `mcp__semble__.find_related` 时，参数中始终包含 `repo`。
