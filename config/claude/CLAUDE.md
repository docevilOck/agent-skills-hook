...
- 需要大范围代码搜索、跨目录定位、日志/文档/搜索结果会污染主上下文，或目标文件不明确时，优先使用 `Explore` 子代理做只读调查。
- 需要实现前拆方案、比较设计路径、定义验证检查点，或任务跨多个模块时，优先使用 `Plan` 子代理先产出实施计划。
- 需要多步通用执行、批量整理、运行命令并汇总结论，且不需要主上下文保留全部中间细节时，优先使用 `general-purpose` 子代理。
- 代码审阅默认交给只读子代理执行；主代理负责核对发现、补充验证和最终输出。
- 仅当任务很小、目标位置明确、无需额外搜索或隔离上下文时，才由主代理直接完成。
