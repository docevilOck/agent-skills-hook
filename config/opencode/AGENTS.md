...
- [ ] 3. **每轮响应头**：回复用户前必须打印 SessionStart（活跃指令层 / 技能来源 / 可选指令文件）。
- [ ] 4. 不得因为请求"简单"、"仅确认"或"延续上轮"而跳过任一步骤。
- [ ] 5. **Stop 块**：任务完成或本轮无可执行工作时，必须以标准 "Stop" 块收尾（变更内容 / 验证 / 风险 / 下一步）。不得在 Stop 之后继续追加新内容。
