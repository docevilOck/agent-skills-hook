...
description: |
  ...
  当用户想要系统性研究一个产品、公司、概念、技术或人物时使用。核心是双轴分析：纵轴追踪从诞生到当下的完整生命历程（以叙事故事呈现），横轴在当下时间截面上与竞品/同类进行系统性横向对比，最后交叉两条轴产出独到洞察。默认产出Markdown研究报告，用户明确要求时可额外生成排版精美的PDF。
...
你正在执行一次横纵分析法深度研究。默认产出一份**Markdown研究报告**，用户明确要求时可额外生成排版精美的PDF。
...
2. **PDF转换（可选）**：如用户明确要求PDF格式，本Skill自带 `scripts/md_to_pdf.py`（基于WeasyPrint）可在Markdown完成后转为排版精美的PDF。依赖安装：`pip install weasyprint markdown --break-system-packages`。⚠️ Windows用户需额外安装GTK3运行时（下载：https://github.com/tschoonj/GTK-for-Windows-Runtime-Environment-Installer），如环境不便安装，向用户说明后交付Markdown即可。
...
最终报告要兼具研究报告的严谨和卡兹克式可读性，让读者能从头读到尾。
...
报告写完后，将完整Markdown文件保存为 `[研究对象名称]_横纵分析报告.md`，直接交付给用户。
...
2. 安装依赖：`pip install weasyprint markdown --break-system-packages`
   - ⚠️ **Windows用户**：WeasyPrint需GTK3运行时（下载：https://github.com/tschoonj/GTK-for-Windows-Runtime-Environment-Installer），无法安装时向用户说明后交付Markdown即可
3. 运行转换脚本：
   ```bash
   python [skill目录]/scripts/md_to_pdf.py input.md output.pdf --title "研究对象名称" --author "数字生命卡兹克"
   ```
4. PDF文件命名为 `[研究对象名称]_横纵分析报告.pdf`，保存到用户工作目录中

脚本已内置完整排版方案（A4页面、封面页、配色方案、页眉页脚等），无需手动调整CSS。生成PDF前Markdown的 `# 标题` 会被自动提取为封面标题。---
