...
- User wants multi-perspective consensus on a plan -- `--consensus`
...
- User wants to start coding immediately with a clear task -- use `ddev-exec` or delegate to executor
...
| Consensus | `--consensus` | Planner -> Architect -> Critic loop until agreement with RALPLAN-DR structured deliberation (short by default, `--deliberate` for high-risk); outputs plan by default |
...
3. **Gather codebase facts first**: Before asking "what patterns does your code use?", spawn the built-in `explore` agent to find out, then ask informed follow-up questions
...
6. **Apply improvements**: When reviewers approve with improvement suggestions, merge all accepted improvements into the plan file before proceeding. Final consensus output **MUST** include an **ADR** section with: **Decision**, **Drivers**, **Alternatives considered**, **Why chosen**, **Consequences**, **Follow-ups**. Specifically:
   ...
   e. Before any execution handoff, derive an explicit **available-agent-types roster** from the known prompt catalog and add concrete **follow-up staffing guidance** for the chosen execution lane (recommended roles, counts, suggested reasoning levels by lane, and why each lane exists)
   f. For coordinated execution, add an explicit launch-hint block with concrete commands and a **verification path** for the chosen lane
7. On Critic approval (with improvements applied): *(--interactive only)* If running with `--interactive`, use `AskUserQuestion` to present the plan with these options:
   - **Approve and execute** — proceed to implementation via the chosen execution workflow
   - **Approve and implement via coordinated execution** — proceed to implementation via parallel agents
...
9. On user approval (--interactive only):
   - **Approve and execute**: hand off the approved plan path as context plus the explicit available-agent-types roster, suggested reasoning levels, concrete role allocation guidance, and direct launch hints for the selected execution workflow. Do NOT implement directly in the planning agent.
   - **Approve and implement via coordinated execution**: hand off the approved plan path as context plus the explicit available-agent-types roster, suggested reasoning levels, concrete staffing / worker-role allocation guidance, and the verification path. Do NOT implement directly.
...
- Verification Steps
- For consensus: **RALPLAN-DR summary** (Principles, Decision Drivers, Options)
- For consensus final output: **ADR** (Decision, Drivers, Alternatives considered, Why chosen, Consequences, Follow-ups)
- For consensus execution handoff: **Available-Agent-Types Roster**, **Follow-up Staffing Guidance** (including suggested reasoning levels by lane), explicit launch hints, and **Verification Path**
...
- Use the built-in `explore` agent (LOW tier, bounded quick pass) to gather codebase facts before asking the user
...
- In consensus mode with `--interactive`, on user approval **MUST** invoke the selected execution workflow for execution (step 9) -- never implement directly in the planning agent
- In consensus mode, execution follow-up handoff **MUST** include an explicit available-agent-types roster plus concrete staffing / role-allocation guidance grounded in that roster, suggested reasoning levels by lane, explicit launch hints, and a verification path
</Tool_Usage>
...
```
Planner: [spawns built-in explore agent: "find authentication implementation"]
...
```
Why bad: The planner should spawn the built-in explore agent to find this, not ask the user.
</Bad>
...
- If the user says "just do it" or "skip planning", **MUST** transition to execution mode. Use `ddev-exec` for implementation handoff. Do NOT implement directly in the planning agent.
...
The separate `/planner` and `/review` workflows have been merged into `$plan`. All workflows (interview, direct, consensus, review) are available through `$plan`.
</Advanced>
