...
## Modification Basis Hard Gate

**硬性门禁：每次执行提交前，必须先向用户询问本次提交的修改依据。此后所有门禁（版本号确认、固件产物、README）方可执行。不存在例外。**

### 执行流程

1. 向用户提问，提供选项的同时允许自定义输入（custom 模式，自由输入）：
...
2. 若用户在自由输入中填入内容（如工单号、需求编号），以用户输入作为修改依据；否则以用户选择的选项作为修改依据。
3. 修改依据必须写入 commit message 正文中。
4. 不得在未获得用户回复修改依据的情况下继续执行提交流程。
...
**硬性门禁：所有版本号相关字段确定后、提交前，必须使用 `question` 工具将即将使用的版本号展示给用户确认。未获得用户确认不得提交。**

展示内容必须覆盖版本号出现的所有位置：

1. 使用 `question` 工具，以 select 形式列出即将写入的版本号：
   - 版本宏值：`VERSION_MAJOR=x VERSION_MINOR=y VERSION_TEST=z`（含 VERSION_BETA 值）
   - 固件文件命名：`<型号>_Vx.yy.zz_<后缀>`（.bin / .dfu 等）
   - changelog 版本记录：`版本：<型号>_Vx.yy.zz_<后缀>`
   - commit message 正文中的版本号
2. 等待用户确认后再继续提交流程。用户选择"确认"才继续，选择"有问题"则返回修正。
3. 此门禁对 commit 和 amend 一视同仁，不可跳过。
...
2. 运行仓库真实构建命令生成新产物。
3. **加密变体优先门禁（强制）**：构建完成后，必须先检查构建输出目录产物，再执行后续命名和复制步骤。此门禁不可跳过，不可先归档后补确认。
   - **列出构建输出目录产物**：检查是否存在加密变体文件（文件名含 `encrypt`、`enc` 或 `_e` 标记）。
   - **若存在加密变体**：必须选择加密版本归档。使用 `question` 工具向用户确认："构建产物包含以下变体：[加密版: `<encrypt_file>` (<size> bytes)] [非加密版: `<plain_file>` (<size> bytes)]，按规则将归档加密版本。" 用户确认后继续；未获确认不得归档。
   - **若无加密变体**：使用唯一产物归档，无需确认。
4. 根据版本宏、构建宏和历史命名规则计算归档文件名。
5. 将构建产物复制到对应历史目录。
6. 若同一版本需要多个变体，逐个构建、逐个命名、逐个归档。
7. 归档前确认目标文件是否已存在。若目标文件是当前版本既有归档，且用户或仓库流程要求"更新当前版本产物"，可用最新构建产物覆盖并记录验证证据；若版本/变体/目标文件不明确，或覆盖跨版本、跨变体、历史冻结产物，必须先询问用户。
8. 归档产物应进入 commit，除非仓库规则明确忽略产物。
9. 即使本次不递增版本，只要存在会进入固件镜像的代码或构建输入改动，默认也要构建并更新当前版本对应的固件产物。只有仓库规则、用户明确要求或历史证据明确证明本次没有固件记录要求时，才可不归档；跳过时必须说明证据。禁止用"未递进版本""只是 amend""只是 fix"作为跳过理由。
10. amend 场景下若版本号、版本命名字段、构建宏、产物命名规则、README/changelog 汇总范围或任何构建输入发生变化，必须把当前版本对应的归档产物视为失效，重新构建、重新复制/覆盖归档文件，并把更新后的产物与文档一起纳入 amend。
...
1. 确认用户确实要求提交。
2. **硬性门禁：询问修改依据**（参见 Modification Basis Hard Gate 章节）。未获得用户回复前不得继续。
3. 读取仓库规则：AGENTS、release 配置、历史提交、版本文件、changelog 文件、产物目录。
4. 硬性门禁：若存在任何代码改动（即使未递增版本），按序执行：
   a. 重新编译目标变体（执行仓库真实构建命令）
   a2. **加密变体检查**：列出构建输出目录，检测 `*encrypt*`/`*enc*`/`*_e*` 文件。若存在加密变体，执行 Firmware Artifact Rules 第 3 条确认门禁（`question` 工具确认）。
   b. 将构建产物按历史命名规则复制到归档目录
   c. 验证产物已存在于归档目录中
   此门禁对 commit 和 amend 一视同仁，不可跳过、不可推迟到后续步骤。
5. 分别确认本次是否触发版本递增、README/changelog 汇总和固件归档；版本递增不是固件归档的前置条件。
6. 若触发版本递增：
    - 更新版本源文件。
    - 构建目标变体。
    - 按历史命名复制/归档产物。
...
8. 若本次是 amend：存在任何代码改动，或涉及版本号/构建输入/产物命名等变化的，必须重新构建目标变体→复制产物到归档目录→验证归档→重新暂存。不可假设前次 commit 的产物仍然有效。
9. 运行验证：diff check、必要的 LSP/构建/测试、产物存在性和文件名检查。
10. 暂存源码、版本文件、changelog、归档产物、release 配置文件；如果是 amend，必须确认重编译生成的新产物已重新进入暂存区。
11. **版本号确认门禁**：使用 `question` 工具将即将写入的版本号展示给用户确认（参见 Version Bump Rules > 版本号确认门禁）。未获用户确认不得继续。
12. 使用仓库模板创建 commit；commit message 只写实际变更，不写"执行了默认固件/README 同步检查""不递进版本""不归档固件"等流程话术。
13. 提交后验证工作区状态、最新 commit message、Change-Id（若 Gerrit hook 存在）。
...
- 未先询问用户修改依据就直接执行提交流程。
- 版本号确认门禁被跳过，未将最终版本号展示给用户确认就提交，导致版本号写错。
- 构建产出同时存在加密和非加密变体时，未识别加密版本，错误归档了非加密固件。
- 构建产出存在加密变体时，未用 `question` 工具向用户确认加密变体文件名及大小，直接归档（跳过了加密变体优先门禁）。
...
- [ ] 已确认用户要求提交。
- [ ] **已向用户询问本次提交的修改依据并获得回复**（Modification Basis Hard Gate）。修改依据已写入 commit message 正文。
...
- [ ] **已执行加密变体门禁**：构建后已检查输出目录产物；存在加密变体时已用 `question` 工具向用户确认加密变体文件名及大小，并选择加密版本归档。
- [ ] **已通过版本号确认门禁**：使用 `question` 工具将版本宏值、固件文件名、changelog 记录和 commit message 中的版本号展示给用户并获得确认。
- [ ] 已运行必要验证并记录结果。
