...
10. 当构建产出包含加密变体和非加密变体时（如文件名含 `encrypt`、`enc` 标记的产物），默认选择加密版本作为归档固件。非加密版本为次要产物，仅在无加密变体或用户明确指定时使用。归档前须向用户确认：已识别到加密变体，正在使用加密版本。
...
- [ ] 若构建产出含加密变体（文件名含 `encrypt`/`enc`），已默认选择加密版本归档，并向用户确认。
