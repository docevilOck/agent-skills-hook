...
在进行代码修改（新增、重构、修复）前，先出 spec 图，用图讲清改动边界、模块关系、接入点和主流程，再进入编码。本 skill 仅用于**代码改动场景**，项目的整体架构文档初始化请使用 `project-architecture-init`。

**边界声明**：本 skill 仅产出设计文档，不得直接修改代码。所有代码修改必须在本 skill 产出 spec 文档并获得确认后，通过 `writing-plans` 拆解执行计划，再由执行计划驱动编码实现。
...
对于 C 项目（`.c` / `.h`），本阶段必须加载 `c-pro` skill，将其中的设计规范、命名规范、注释规范作为 spec 约束一并写入文档，不得留到编码阶段临场决定。
...
5. 若无需 PC 测试或跳过步骤 4，进入 `detail-workflow` 继续细化结构体和数据流；若步骤 4 已产出测试，仍可按需进入 detail 补充实现细节。
6. 再进入 `writing-plans`，把已确认设计拆成执行计划。
7. 实现完成并拿到验证证据后，默认进入 `final-gate`，按图对照代码实现是否一致。若步骤 4 产出了测试 demo，`final-gate` 阶段必须跑通该 demo。
...
如果目标是 C 项目（`.c` / `.h`），必须先加载 `c-pro` skill，本阶段把以下约束写进 spec 文档，而不是留到编码时临场决定：
...
- 模块命名、类型命名（`_t` 后缀）、API 前缀、错误码枚举必须按 `c-pro` 规范提前约定
...
- 错误码和异常出口是集中处理还是分层返回
- 模块公开 API 命名前缀、类型 `_t` 后缀、宏命名规则是否已按 `c-pro` 规范约定
...
- 命名规范、Doxygen 注释要求是否已按 `c-pro` skill 在 spec 约束中明确
