...
| `ddev-spec` | 代码修改前编写 spec 文档，用图表达改动边界、模块关系、接入点和主流程。默认一份文档，仅两个完全不同的改动面才拆分。仅用于代码改动场景。 | 自动 — 代码修改前需要 spec 文档 |
| `ddev-detail` | spec 文档确认后，以图优先方式梳理结构体定义、数据流和流程。仅用于代码改动场景。 | 自动 — spec 文档确认后需要细化 |
| `ddev-gate` | 代码实现完成后验收：大改动并行派发架构一致性审查 + 代码评审（规范/质量/注释/清理）两个 subagent；小改动 1 个整合 subagent；偏离修正或记入实现笔记。 | 自动 — 实现完成后验收 |
...
② ddev-detail                       ← 细化实现细节：结构体定义、数据流、关键流程
  ...
  ▼ 代码写完
③ ddev-gate                 ← 最终验收门禁
     ├─ 大改动（按改动面路由）：同一轮并行派发两个独立 subagent
     │    ├─ 一致性审查：只核对代码与文档规划架构的偏离
     │    │              ├─ 可修正 → 修正到一致
     │    │              └─ 不可修正/不修正 → 写入 implementation-notes 的 Deviations
     │    └─ 代码评审：合并 编码规范 + 代码质量 + 注释 + 清理清单（只读）
     │  只要发生过代码修改 → 基于最终代码重跑两个只读复审
     │  最终以同一份代码上两个审查都 pass、且该轮无代码修改为准
     └─ 小改动（compact）：1 个独立 subagent 一次性完成 一致性 + 代码评审
```

| 阶段 | 技能 | 核心职责 | 关键约束 |
|------|------|----------|----------|
| ① 设计 | `ddev-spec` | 在编码前用图讲清改动边界、模块关系、接入点和主流程 | 仅产出 spec 文档，不得直接改代码；图优先、文辅佐；一次改动默认一份 spec，仅真正不同的改动面才拆分；遵守项目级 `docs/architecture/` 约束 |
| ② 细化 | `ddev-detail` | spec 确认后细化结构体定义、数据流图和关键流程图 | spec 文档必须先获确认才能进入此阶段；图优先、文辅佐 |
| ③ 验收 | `ddev-gate` | 大改动并行派发一致性审查 + 代码评审两个独立 subagent；小改动 1 个整合 subagent | 一致性只审架构偏离，偏离修正或记入实现笔记；代码评审合并规范/质量/注释/清理清单；最终以同一份最终代码上的只读双 pass 为准 |

> **联动技能**：`ddev-diagram`（画图规范）、`ddev-plan`（拆解执行计划，只规划执行步骤）、`ddev-exec`（执行计划，中途不设审查阶段）、`ddev-clean`（gate 中负责清理项识别，执行由主 agent 按需进行）、`ddev-c-pro`（C 项目 gate 合并评审的规范+质量维度）、`ddev-comment-gen`（C 项目 gate 合并评审的注释维度）。

## 三、项目管理

| 技能 | 说明 | 触发方式 |
|------|------|----------|
| `plan` | 战略规划与可选访谈式澄清流程，帮助梳理需求和方案。 | 自动 — 需要制定计划时 |
| `ddev-exec` | 按已写好的实现计划顺序执行任务，先做批判性检查，再依次执行并收尾。 | 自动 — 有实现计划需要执行时 |
...
- **触发词**：用户手动调用（无特定触发词约束）
- **说明**：初始化项目级长期架构规范文档。建立模块边界、依赖规则和变更门禁，约束后续代码修改范围。每个模块独立成文，讲清职责、边界、允许依赖和对外暴露接口。
- **使用示例**：
  ```
  帮我初始化项目架构文档
  ```
