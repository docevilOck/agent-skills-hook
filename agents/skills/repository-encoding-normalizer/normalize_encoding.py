...
# C 转义序列 → 实际控制字节映射
# 当原始 GBK 源码中的字符串字面量包含 \r \n \t \\ \" 等 C 转义时，
# 逐字节扫描需要识别这些序列并输出对应控制字节，而不是字面 \x5C\x72。
C_ESCAPE_MAP = {
    0x6E: 0x0A,  # \n → LF
    0x72: 0x0D,  # \r → CR
    0x74: 0x09,  # \t → TAB
...
def gbk_bytes_to_hex_escapes(raw_bytes, source_encoding="gbk"):
    r"""
    将原始编码字节序列转为 \\xHH 转义字符串。
    自动识别并保留 C 转义序列（\\r \\n \\t \\\\ \\\" 等）为实际控制字节。

    参数:
        raw_bytes: 原始编码的字节序列
        source_encoding: 原始编码名称（仅用于错误提示）
    ...
    while i < len(raw_bytes):
        b = raw_bytes[i]
        # 检测 C 转义序列: 0x5C 是反斜杠
        if b == 0x5C and i + 1 < len(raw_bytes):
            next_byte = raw_bytes[i + 1]
            if next_byte in C_ESCAPE_MAP:
                # 是 C 转义序列，输出实际控制字节
                actual = C_ESCAPE_MAP[next_byte]
                result.append("\\x{:02X}".format(actual))
                i += 2
                continue
            elif next_byte == 0x78 and i + 3 < len(raw_bytes):
                # \xHH 形式的 hex 转义（如原始代码中就有 \xFF 写法），保持原样
                hex_digits = raw_bytes[i + 2 : i + 4]
                try:
                    hex_val = int(hex_digits.decode("ascii"), 16)
                    result.append("\\x{:02X}".format(hex_val))
                    i += 4
                    continue
                except ValueError:
                    pass
...
def bytes_from_source_string(text, source_encoding="gbk"):
    """
    从原始源码字符串中提取字节序列，支持识别 C 转义和 hex 转义。
    用于从 scan_encoding.py 输出的 old_string 还原为原始字节。
    ...
    while i < len(text):
        ...
        if c == "\\" and i + 1 < len(text):
            ...
            if next_c in "rnt0\"'\\":
                # C 转义序列
                result.append(C_ESCAPE_MAP.get(ord(next_c), ord(next_c)))
                i += 2
                continue
            elif next_c == "x" and i + 3 < len(text):
                # \xHH 形式
                try:
                    ...
                    has_warning = True
        result.append(ord(c) if ord(c) < 128 else c.encode(source_encoding, errors="replace")[0])
        i += 1
    return bytes(result), has_warning
