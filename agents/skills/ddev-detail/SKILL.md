...
- **结构体复用检查**：在定义任何新增结构体之前，必须搜索已有结构体中是否存在可管理同类状态的候选——
  - 搜索包含同名/近名字段的已有结构体（grep 字段名）
  - 搜索已有 context/handle 结构体，判断新增状态是否可直接作为其成员
  - 搜索同模块已有的 `_t` 类型定义，避免创建功能重叠的新类型
  结果写入 detail 文档的结构体来源标注中：每个"新增"结构体必须附带"复用/扩展已有不可行的理由"（一句话即可，如"生命周期不同""所属模块不同"）。
- 相关时序图、流程图、接口文档（如有）
...
- **状态归属去重**：在收敛状态前，先确认同类状态是否已被已有 context 管理。如果已有 context 已管理打印机状态，新增字段应优先扩展该 context，不得创建"第二个打印机状态管理者"（无论是新 context 还是新全局变量）。
- **函数功能去重**：对每个新增函数，先搜索已有代码中是否存在功能等价的实现；如果存在可扩展的已有函数，标注为"扩展已有"而非"新增"。
...
- 结构体成员注释使用 Doxygen 行内格式（`/**< */`，必须用中文，参照 `ddev-comment-gen` skill），公开 API 的 `@brief` / `@param` / `@return` 必须使用中文
- 类型名统一 `_t` 后缀，公开 API 统一模块前缀，宏全大写 + 模块前缀，私有函数用 `static`
- 结构体/缓冲区声明中的大数字（≥1024）必须写成 `N * 1024` / `N * 1024 * 1024` 可读表达式并命名宏（如 `MOD_TX_BUF_SIZE (16 * 1024)`），禁止在结构体定义或示例代码里裸写大数
...
8. **结构体去重**：每个标注为"新增"的结构体，是否已确认无已有结构体可管理同类状态？是否有"扩展已有结构体"被误判为"必须新增"？
9. **全局变量合理性**：detail 文档中是否引入了文件级/全局级变量？如有，每个是否已给出无法收敛到已有 context / 局部变量的理由？
10. **依赖方向**：新增字段/函数的归属模块是否正确？是否出现在 A 模块的 context 中管理 B 模块的专属状态？是否出现底层结构体引用上层类型？
