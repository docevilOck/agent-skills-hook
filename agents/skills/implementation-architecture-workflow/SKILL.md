...
如果某件事容易和本次改动混淆，不要写“这个不做”；改成正向表述，明确这次实际要落地的边界、入口、流程、结构和验证对象。
...
2. 再用这个 skill 把本次实现架构文档写出来或改完，并记录遵守了哪些项目级约束。
3. 再交给 `diagram-workflow`，把同一份架构文档细化成图，并决定哪些图可以自动 ASCII，哪些图必须手写 ASCII。
4. 架构文档和图都确认后，再进入 `writing-plans`，把已确认的设计拆成执行计划。
5. 实现完成并拿到验证证据后，默认进入 `implementation-final-gate`，核对代码实现是否与 architecture/detail 文档和项目级架构规范一致，再决定是否可以正式收尾。
...
2. 如果存在 `docs/architecture/`，先做项目级架构越界检查：
   ...
   - 是否触发项目级"架构变更门禁"
3. 先写架构文档里的图和说明。
4. 按 `diagram-workflow` 决定展示图策略：复杂图默认手写 ASCII，简单图才可参考自动 ASCII 渲染。
5. 检查架构文档是否清楚、是否过挤、是否容易比较。
6. 在每张 ASCII 图附近标注对应 `.puml` 源路径。
7. 如果看不清，先判断是否该改成手工 ASCII，而不是默认重新渲染。
