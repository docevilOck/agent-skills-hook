...
计划里只允许出现**实现、验证、记录**三类执行步骤。

禁止出现：

- 划分阶段或任务后，额外设置“审查阶段”“review 阶段”“验收阶段”“复核阶段”；
- 把派发审查 subagent、执行代码审查、一致性验收、注释审查、清理单独列为任务或步骤；
- 用“审查通过”作为任务完成或进入下一阶段的前置条件。

任务内的验证和自检属于执行动作，可以保留，但不得单独成阶段，也不得写成对全部产出的正式审查。

所有审查统一由 `ddev-gate` 在收口时完成：gate 会同时派发一致性审查和代码评审（编码规范/质量/注释/清理合并为一个）两个独立 subagent。计划不得提前承担、安排或转述这些审查动作。
...
默认一份确认过的 spec 对应一份执行计划。

只有当 spec 已经拆成多份、且各改动面属于完全不同的子系统时，才为每份 spec 各写一份计划；每份计划独立产出可运行、可测试的软件。若 spec 本身是一份合并文档（同一改动的多个模块、入口或流程），就只写一份计划，不得按模块、文件或技术层自行拆计划。
...
2. **测试未覆盖部分（直接开发）**：硬件依赖、中断时序、板级外设等测试无法覆盖的部分，按常规实现 → 验证 → 任务内自检编排
...
3. **目标函数结构确认**：对计划中要修改的每个函数——
   - 用 `Read` 读取目标函数的**完整代码**（不是 grep 片段）
   - 确认完整 if-else 分支结构、已有的错误/异常处理块、return 点
   - 在此结构基础上设计修改插入点
   - 禁止仅凭 grep/explore 的片段信息决定插入位置
4. **TDD 任务 API 验证**：TDD 任务的测试代码引用的 API 签名必须——
...
- 一个任务内可以包含”阅读文档 -> 实现 -> 验证 -> 任务内自检”这一组完整动作
...
对照引用文档做本任务的实现自检：结构、状态迁移、接口语义和流程节点是否一致；若不一致，先修正代码或回上游补文档。这是执行动作的一部分，不是独立审查阶段。
...
**11. 执行步骤纯净化：** 计划里是否只有实现、验证、记录三类执行步骤？是否出现了独立审查阶段、审查任务、派发审查 subagent 的安排，或把"审查通过"写成任务完成条件？有就删掉，审查统一交给 `ddev-gate`。
...
- 使用 `ddev-exec`
- 按计划顺序逐步执行
- 执行阶段不安排中途审查；所有审查统一由 `ddev-gate` 收口
...
2. 进入 `ddev-gate`：gate 会同时派发两个独立 subagent——一致性审查 + 代码评审（编码规范/质量/注释/清理合并为一个）。
3. 如果 gate 返回 `blocked` / `need-info`，先修复或补齐缺失输入，再重新进入 `ddev-gate`，直到两个审查在同一份最终代码上都 `pass`。
4. 代码偏离文档规划的架构时，修正到一致；无法修正或决定不修正的偏离，写入 `implementation-notes.md` 的 Deviations（偏离点、理由、影响范围）。
5. 只有 `ddev-gate` 通过后，才能宣称“实现已经按设计完成”。
