...
任务拆完后，必须做并行度分析。目标是最大化并行执行的独立任务数。
...
依赖关系来源于：
- **文件冲突**：两个任务修改同一文件 → 后者依赖前者
- **接口/类型依赖**：Task B 使用了 Task A 定义的结构体/枚举/函数 → B 依赖 A
- **逻辑顺序**：Task B 的验证依赖 Task A 的产物（如必须先有 context 结构体才能写状态机）

### 并行判定规则

满足以下**全部条件**的任务可以放入同一 wave 并行执行：

1. **无相互依赖**：不在彼此的 `depends_on` 链上
2. **文件不重叠**：修改/新建的文件集无交集
3. **接口无耦合**：不依赖对方定义的 struct/enum/函数签名

### Wave 分组

按依赖拓扑排序，将无依赖关系的任务归入同一 wave。Wave 之间串行（后一 wave 依赖前一 wave 的全部完成），Wave 内部并行（所有任务独立派发子代理）。

```
Wave 1（无依赖）:  Task 1, Task 2      ← 并行派发 2 个子代理
Wave 2（依赖 W1）: Task 3, Task 4, T5   ← 并行派发 3 个子代理
Wave 3（依赖 W2）: Task 6               ← 单代理
```
...
| Wave | 任务 | 依赖 | 文件（不重叠） | 并行 |
|------|------|------|--------------|------|
| 1 | Task 1: 错误码枚举 | 无 | `errors.h` | ✓ |
| 1 | Task 2: 上下文结构体 | 无 | `context.h` | ✓ |
| 2 | Task 3: 状态机实现 | Task 1,2 | `state.c` | — |
| 2 | Task 4: API 接口 | Task 2 | `api.c` | — |

> Wave 1 内 2 个任务可并行派发子代理执行。Wave 2 必须等 Wave 1 全部完成后开始。
````
