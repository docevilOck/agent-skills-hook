...
对于 C 项目（`.c` / `.h`），本阶段必须加载 `ddev-c-pro` skill，将其设计规范、命名规范、Doxygen 注释标准、风格偏好作为实现约束写入计划，确保执行者明确知道编码规范要求。
...
- `ddev-c-pro` skill：编码规范、命名规范、Doxygen 注释标准（C 项目必填）
...
- C 项目须遵守 `ddev-c-pro` skill：`_t` 类型后缀、模块前缀、Doxygen 注释、显式返回码、所有权明确
...
- 按 `ddev-c-pro` skill 要求补 Doxygen 注释、遵循命名规范和风格偏好
...
**6. 代码路径存在性：** 计划中列出的所有文件路径（新建/修改）是否真实存在于仓库中？新建文件的父目录是否存在？
...
**7. TDD 覆盖核对：** 如果存在 `ddev-pc-test` 产出的测试用例：
