...
对于 C 项目（`.c` / `.h`），本阶段必须加载 `ddev-c-pro` 和 `ddev-comment-gen` skill，将设计规范、命名规范、Doxygen 注释标准、风格偏好作为实现约束写入计划，确保执行者明确知道编码规范要求。
...
> **给代理型执行者：** 必需子技能：使用 `superpowers:ddev-exec` 按任务逐步实现这个计划。步骤使用复选框（`- [ ]`）语法跟踪。
...
- `ddev-c-pro` skill：编码规范、命名规范（C 项目必填）
- `ddev-comment-gen` skill：Doxygen 注释标准（C 项目必填）
...
- C 项目须遵守 `ddev-c-pro` skill：`_t` 类型后缀、模块前缀、显式返回码、所有权明确
- C 项目须遵守 `ddev-comment-gen` skill：Doxygen 注释完整性（公开 API/@file/结构体成员）
...
**9. TDD 覆盖核对：** 如果存在 `ddev-pc-test` 产出的测试用例：
...
3. 最后进入 `ddev-gate`，按 spec 文档、detail 文档、代码实现、验证证据做最终一致性验收；C 项目还会经 `ddev-c-pro` 规范审查。
