---
name: ddev-arch
description: 【禁止自动加载 — 不可被「技能强制评估」自动匹配】仅限用户手动调用。初始化项目级长期架构规范与嵌入式硬件基线文档，建立模块边界、依赖规则、机型/分区/外设/GPIO 基线和变更门禁，约束后续代码修改范围。
---
...
- **本 skill**：初始化项目级长期架构规范，产出稳定的模块边界、依赖规则、硬件/构建基线和变更门禁。它约束后续所有代码修改。
- **`ddev-spec`**：针对某一次代码改动，编写临时实现架构文档。它必须读取并遵守本 skill 产出的项目级架构规范。

## 触发方式

- **禁止自动加载**：不参与「技能强制评估」自动匹配
- **仅限用户手动调用**：只有用户明确要求初始化项目级架构/硬件基线文档时才允许进入
...
1. 按"探索要求"探索仓库，先判定是否为多独立代码单元仓库（见"模块边界识别"规则 0），再形成模块候选、职责候选和依赖候选
2. **多单元仓库**：先写顶层 `00_仓库集成架构.md`（按"仓库集成架构模板"），再为每个独立代码单元写各自的 `01_架构总览.md` 和模块文档。**单单元仓库**：直接写 `01_架构总览.md`（按"单单元架构总览模板"）。**技术栈字段必须填写实际检测到的语言**，后续步骤以此为分流依据
3. 为每个模块写独立规范文档，按"模块规范模板"编写；多单元仓库时模块文档放在对应单元的子目录下
4. 需要图示时，按 ddev-diagram 规范手写 ASCII 到 `.md`，图附近标注用途说明
5. **项目依赖与封装审计**：根据各单元 `01_架构总览.md` 中声明的技术栈，按 [lang-audit-matrix.md](references/lang-audit-matrix.md) 分流到对应的审计策略（详见下方"项目依赖与封装审计"章节）。多单元仓库时，每个单元独立产出审计文档（放在各自子目录下）
6. **构建变体与环境审计**：根据各单元 `01_架构总览.md` 中声明的技术栈，按 [lang-audit-matrix.md](references/lang-audit-matrix.md) 分流到对应的审计策略（详见下方"构建变体与环境审计"章节）
...
0. **独立代码单元检测**（最高优先级）：先判断仓库是否包含多套独立编译、无源码交叉引用的代码单元——
   - **异构多核**：同一芯片的不同核心（如 ARM A7 + RISC-V E907），各自有独立 OS、独立构建系统和入口
   - **多服务/多应用**：同一仓库下多个独立服务或应用，各自有独立构建配置（`CMakeLists.txt`、`Makefile`、`go.mod`、`Cargo.toml` 等）且互不直接源码依赖
   - **多固件镜像**：同一产品线多个独立固件镜像（如 bootloader + RTOS + Linux kernel），各自独立构建
   检测方法：扫描仓库顶层目录，若存在多个不共享构建系统的独立子目录（各自包含独立入口和构建配置），且它们之间无源码级 `#include`/`import` 交叉引用，则判定为多个独立代码单元。**多单元场景下，每个单元独立产出架构文档（各自的结构总览+模块边界），顶层另出 `00_仓库集成架构.md` 描述单元间的交互、数据流和联合构建关系。单单元场景直接走后续 1-7 条规则检测模块边界。**
...
对于嵌入式 C/C++ 项目，本步骤除了把机型宏、构建命令和产物路径注入 `CLAUDE.md`，还要补齐当前机型的硬件资源规格文档，形成后续实现和审查共同引用的硬件基线。

#### 硬件基线文档（嵌入式项目必做）

在机型宏扫描完成后，继续从当前项目中提取并整理以下信息，写入 `docs/architecture/hardware/<MODEL>-hardware-spec.md`：

- 主控与计算资源：MCU 型号、内核架构、核数、主频、片内 SRAM/ROM、I-Cache/D-Cache
- 存储资源：片外 Flash / PSRAM 的容量、接口类型、频率，以及固件实际烧录介质
- Flash 分区布局：分区名称、偏移、大小、用途，以及固件写入起始偏移和最大可用空间
- 外设与通信接口：UART / SPI / I2C / I2S / USB / WiFi / 蓝牙 / 4G / 以太网等绑定关系
- 执行机构、传感器、音频、人机交互、电源与系统控制 GPIO
- GPIO 汇总表：GPIO 号、功能、方向、复用说明，过滤 `PRT_GPIO_UNVALID` / `-1` / `0xFF` 等无效值
- 如存在多机型条件编译分支，补一节机型差异对比，明确当前机型与相邻变体的主要差异

搜索和整理时遵守以下规则：

- 所有路径都通过 `find` / `grep` 或等效检索确认，不假设文件名
- 机型以当前活跃的非注释 `#define` 为准；若用户明确指定目标机型，以用户指定为准
- 芯片 datasheet 中的片内 SRAM / ROM 等常数可直接写入，无需从配置文件反推
- 分区表解析要跳过 `#` 注释行，正确处理 hex 偏移值
- 文档统一使用 KB / MB / MHz 等物理单位；必要时附原始配置值
...
- 新增/删除/合并/拆分模块时，更新对应单元的架构总览模块列表，新增/删除对应模块文档，同步更新 `AGENTS.md` 或 `CLAUDE.md`（按写入策略判断目标文件）
- **新增/删除独立代码单元时**，更新 `00_仓库集成架构.md` 的单元清单和交互关系，新增/删除对应单元子目录下的全套架构文档（仅多单元仓库）
- **独立代码单元间通信协议或交互关系变更时**，更新 `00_仓库集成架构.md` 的"单元间交互关系"和"跨单元约束"（仅多单元仓库）
