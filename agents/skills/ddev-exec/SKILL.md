...
当所有任务完成并通过计划要求的验证后，向用户报告执行结果摘要（完成任务数、关键验证证据、未覆盖风险）。

**禁止 agent 自动进入默认收尾流程。** 必须等待用户明确确认后再进入收尾。

**提示模板：**
- 告知所有任务已完成
- 列出关键验证证据和未覆盖风险
- 询问：”是否进入默认收尾（ddev-gate 一致性验收）？”
- 在用户明确确认前，不自行进入 gate / clean / review 等后续阶段
...
**ddev-gate 内部的 gate → clean → review 循环为自动化流程，不受此限制。** 但 exec → gate 的入口、gate 通过后的后续动作（如 archive / commit / 发布），**必须等待用户明确确认**，禁止 agent 自动推进。
