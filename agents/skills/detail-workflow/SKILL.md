...
对于 C 项目（`.c` / `.h`），本阶段必须加载 `c-pro` skill，将其设计规范、命名规范、Doxygen 注释标准落实到结构体定义、API 签名和文档约束中。
...
如果目标是 C 项目（`.c` / `.h`），必须先加载 `c-pro` skill，这一步必须把”禁全局变量、禁分支失控、禁职责混杂”落实到结构体和数据流层，同时加入 c-pro 的通用编码规范：
...
- 公开 API 的 Doxygen 注释格式、模块命名前缀、类型 `_t` 后缀、错误码枚举是否已按 `c-pro` skill 在文档中明确
...
- 能直接支撑后续把函数按单一职责拆开
- 命名规范、Doxygen 注释标准、错误码约定已按 `c-pro` skill 在文档中明确，可直接作为编码和审查依据
