...
4. **最小改动**：修复类改动只保留修复本体，不夹带未被要求的打点、统计、日志或重构。新增打点前先确认其日志宏是否受条件编译控制（如 `prt_dbgi` 受 `PRT_DEBUG` 控制、默认注释掉）——默认编译为空即是无效代码；确需观测时按验证期临时补丁处理，验证完即撤，不进入提交。
...
- **改动范围与打点**：修复类改动是否夹带了未要求的统计字段、计数、周期日志或重构；打点日志是否受条件编译控制（如 `prt_dbgi` 依赖 `PRT_DEBUG`、默认关闭），若默认编译为空则属无效代码，应移除或改为验证期临时补丁并在验证后撤销
...
- **死代码与重复**：未被调用的 static 函数、非 static 的无人引用公开符号、不可达分支、跨文件的重复逻辑（DRY 违规）。判定或删除死代码前必须完成全仓验证四件套，禁止仅凭 `src/` 或单个子工程的 grep 结果（仅见定义处一处）下结论：
  - 全仓文本搜索：覆盖所有文件类型与未跟踪文件，并确认 `.gitignore` 未排除源码目录
  - 头文件声明检查：搜索是否有任意头文件声明过该符号
  - 构建产物未定义引用：对 `.o`/`.a`/`.elf` 等构建产物执行 `nm -u`
  - 全量链接反证：执行完整链接，非 `static` 符号若仍有外部引用必报 `undefined reference`
