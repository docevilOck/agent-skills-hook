...
## 影响面评估（推荐使用 codegraph_impact）

在开始逐项审查前，先用以下 CodeGraph 工具评估改动的实际影响范围：

- `codegraph_impact` — 对本次改动涉及的关键符号做影响半径分析，确认：
  - 改动影响了哪些调用方（callers）
  - 改动的波及范围是否与 spec/detail 文档声明的范围一致
  - 是否存在文档未声明的受影响模块（若存在 → `blocked`）
- `codegraph_callers` / `codegraph_callees` — 检查关键 API 的调用链，确认新增/修改的接口是否被预期之外的模块调用
...
- codegraph_impact 分析：[影响半径、调用方数量、是否超出 spec/detail 声明范围]
- 遗漏文件（如有）：[codegraph_impact 发现但不在审查范围内的受影响文件]
